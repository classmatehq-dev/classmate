...
export function Skeleton({ className }: { className?: string }) {
  return (
    <div
      className={cx(
        "animate-pulse rounded-lg bg-light-blue/70",
        className,
      )}
    />
  );
...
/** placeholder that mirrors a FeedPost while the feed loads */
  ...
  return (
    <div className="space-y-3">
      {Array.from({ length: count }).map((_, i) => (
        <div
          key={i}
          className="rounded-card border border-border bg-surface p-4 shadow-card max-md:mx-3"
        >
